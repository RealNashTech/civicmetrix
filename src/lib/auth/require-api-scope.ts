...
export async function requireApiScope(request: Request, requiredScope: string) {
  ...
  if (!token) {
    throw new AuthorizationError(401, "Missing API token.");
  }

  const validated = await validateApiToken(token);
  if (!validated) {
    throw new AuthorizationError(401, "Invalid API token.");
  }

  if (!hasScope(validated.scope, requiredScope)) {
    throw new AuthorizationError(403, "Insufficient API token scope.");
  }
